...
namespace Dashboard.BLL.Services.ImageService
{
    public class ImageService : IImageService
    {
        ...
        public async Task<ServiceResponse> SaveImageUserAsync(IFormFile image)
        {
            if(image == null)
            {
        ...
        public async Task<ServiceResponse> SaveImageProductAsync(IFormFile image)
        {
            if (image == null)
            {
                return ServiceResponse.GetBadRequestResponse(message: "Не вдалося зберегти зображення", errors: "Не знайдено файл");
            ...
            var imageName = Guid.NewGuid().ToString() + "." + ext;
            var rootPath = _webHostEnvironment.WebRootPath;
            var imagePath = Path.Combine(rootPath, Settings.ProductsImagePath, imageName);
